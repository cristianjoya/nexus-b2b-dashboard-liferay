...
import { formatOwnerDisplay } from '../api/accounts';
...
interface TransactionsTabProps {
	...
	restrictToOwnerUserId?: number | null;
}

export function TransactionsTab({
	canFilterByOwner = false,
	permissions,
	restrictToOwnerUserId = null,
}: TransactionsTabProps) {
	const signedIn = isUserSignedIn();
	const { data: accountOptions } = useAccountOptions({
		ownerUserId: restrictToOwnerUserId,
	});
	const accountMap = useMemo(() => {
		const map = new Map<
			number,
			{ label: string; ownerUserId?: number; ownerUserName?: string }
		>();
	...
	const effectiveOwnerUserId =
		restrictToOwnerUserId != null && restrictToOwnerUserId > 0
	...
	const listParams = useMemo(
		...
		[effectiveOwnerUserId, page, search, status, transactionType],
	);
	...
	function resolveOwnerUserName(accountId?: number) {
		...
		return formatOwnerDisplay(account);
	}
