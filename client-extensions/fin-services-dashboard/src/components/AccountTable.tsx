import { formatAccountType, formatOwnerDisplay, type Account } from '../api/accounts';
		...
		>
			{account && (
				<p>
					...
					({account.accountName})? También se eliminarán sus
					transacciones y solicitudes de crédito. Esta acción no se
					puede deshacer.
				</p>
			)}
		</ConfirmDeleteDialog>
										...
										{showAssignedOwner && (
											<td className="px-4 py-4">
												<p className="nexus-title font-medium">
													{formatOwnerDisplay(account)}
												</p>
												{account.ownerUserId != null &&
													account.ownerUserId > 0 &&
													formatOwnerDisplay(account).startsWith(
														'Usuario #',
													) && (
