...
4. Objects importados + Action de saldo (Fases 1–2).
5. En **Server Administration → Script** (Groovy):
   - `scripts/grant-b2b-object-permissions.groovy` (una vez)
   - `scripts/assign-b2b-user-role.groovy` (por usuario; ajustar email)
   - `scripts/seed-demo-data.groovy` (opcional)

Detalle de roles: [objects/ROLES-PERMISSIONS.md](objects/ROLES-PERMISSIONS.md). Seguridad: [SECURITY.md](SECURITY.md).
